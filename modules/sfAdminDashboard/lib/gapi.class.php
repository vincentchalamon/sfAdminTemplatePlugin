...
class gapi
{
   ...
   * @return gapi
   */
  public function __construct($email, $password, $profile, $token=null)
  {
    if($token !== null)
    {
  ...
  /* DISPLAY FUNCTIONS */
  /*********************/
  protected function getGapiRender($dimensionName, $metricName, $package, $function, $name)
  {
    // Load Google main javascript
    use_javascript("http://www.google.com/jsapi");
    // Parse results for javascript
    $datas = "";
    foreach($this->results as $result)
    {
      $dimension = $result->getDimension($dimensionName);
      $metric = $result->getMetric($metricName);
      // Numeric dimension => convert date to correct local format
      if(is_numeric($dimension))
      {
        $dimension = format_date(substr($dimension, 0, 4)."-".substr($dimension, -4, 2)."-".substr($dimension, -2), "d");
      }
      $datas.= "['$dimension', $metric],";
    }
    return sprintf(<<<EOF
<script type='text/javascript'>
  ...
  function %s()
  {
    ...
    {
      data.setValue(i, 0, datas[i][0]);
      data.setValue(i, 1, datas[i][1]);
    }
    %s
  }
</script>
...
EOF
  ...
  /**
   * Display a Google area chart localized by results
   *
   ...
   * @return String Error message or Google area chart
   */
  public function getGapiAreaChart($dimensions, $metrics, $sort = null, $filter = null, $start = null, $end = null, $index = 1, $max = 30)
  {
    $params = func_get_args();
    $this->script = "var chart = new google.visualization.AreaChart(document.getElementById('google_area_chart'));
      chart.draw(data, {height: 200, legend: 'none', pointSize: 4, lineSize: 3});";
    try
    {
      // Retrieve datas
      $this->requestReportData($this->profile, $dimensions, $metrics, $sort, $filter, $start, $end, $index, $max);
      return $this->getGapiRender($dimensions[0], $metrics[0], "areachart", "drawAreaChart", "google_area_chart");
    }
    catch(Exception $e)
    {
      // Error
      if(sfConfig::get('sf_environment', 'prod') == "dev") throw $e;
      return __("Unable to load statistics.", null, "sfMenu");
    }
  }

  /**
   * Display a Google pie chart localized by results
   *
   * @param String $title Element title
   * @param Array $dimensions Google Analytics dimensions e.g. array('browser')
   * @param Array $metrics Google Analytics metrics e.g. array('pageviews')
   ...
   * @param Int $start_index OPTIONAL: Start index of results
   * @param Int $max_results OPTIONAL: Max results returned
   * @return String Error message or Google pie chart
   */
  public function getGapiPieChart($dimensions, $metrics, $sort = null, $filter = null, $start = null, $end = null, $index = 1, $max = 30)
  {
    $params = func_get_args();
    $this->script = "var chart = new google.visualization.PieChart(document.getElementById('google_pie_chart'));
      chart.draw(data, {width: 375, height: 300, is3D: true});";
    try
    {
      // Retrieve datas
      $this->requestReportData($this->profile, $dimensions, $metrics, $sort, $filter, $start, $end, $index, $max);
      return $this->getGapiRender($dimensions[0], $metrics[0], "piechart", "drawPieChart", "google_pie_chart");
    }
    catch(Exception $e)
    {
      // Error
  ...
  /**
   * Display a Google world map localized by results
   *
   * @param String $title Element title
   * @param Array $dimensions Google Analytics dimensions e.g. array('browser')
   * @param Array $metrics Google Analytics metrics e.g. array('pageviews')
   ...
   * @param Int $start_index OPTIONAL: Start index of results
   * @param Int $max_results OPTIONAL: Max results returned
   * @return String Error message or Google world map
   */
  public function getGapiMap($dimensions, $metrics, $sort = null, $filter = null, $start = null, $end = null, $index = 1, $max = 30)
  {
    $params = func_get_args();
    $this->script = "var container = document.getElementById('google_countries');
    ...
    try
    {
      // Retrieve datas
      $this->requestReportData($this->profile, $dimensions, $metrics, $sort, $filter, $start, $end, $index, $max);
      return $this->getGapiRender($dimensions[0], $metrics[0], "geomap", "drawMap", "google_countries");
    }
    catch(Exception $e)
    {
      // Error
  ...
  {
    return str_ireplace("%report_id%", $this->report_id, self::report_url);
  }
...
class gapiReportEntry
{
  ...
  {
    return isset($this->dimensions[$index]) ? $this->dimensions[$index] : null;
  }
  ...
  public function getMetric($index)
  {
