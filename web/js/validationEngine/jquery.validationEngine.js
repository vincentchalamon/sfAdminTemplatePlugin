...
(function($) {
  ...
  $.fn.validationEngine = function(settings) {
    ...
    $(this).bind("submit", function(caller){   // ON FORM SUBMIT, CONTROL AJAX FUNCTION IF SPECIFIED ON DOCUMENT READY
      ...
      $('.submiter').hide();
      $('.frmLoader').show();
      $('.submitFormField .sendForm').hide();
      $('.submitFormField .loading').show();
      if($('#uploadify').length)
      {
        if(fcount > 0)
        {
  ...
  $.validationEngine = {
    ...
    submitForm : function(caller){
      if($.validationEngine.settings.ajaxSubmit){
        ...
        $.ajax({
          ...
          success: function(data){
            if($.validationEngine.settings.overrideSuccess == true){
              $.validationEngine.settings.success(data);
              return;
            }
      ...
      if(!$.validationEngine.settings.beforeSuccess($(caller))){
        if ($.validationEngine.settings.success){	// AJAX SUCCESS, STOP THE LOCATION UPDATE
    ...
    buildPrompt : function(caller,promptText,type,ajaxed,second) {			// ERROR PROMPT CREATION AND DISPLAY WHEN AN ERROR OCCUR
      ...
      $(caller).parent().addClass('error'); // Added by Scalpweb
      ...
      if($(caller).is(':hidden'))
      {
        var prt = $(caller).parent();
        while($(prt).is(':hidden'))
          prt = $(prt).parent();
        $(divFormError).css({
          "top": ($(prt).offset().top-20),
          "left": $(prt).offset().left,
          "marginTop":0,
          "opacity":0
        })
      }
    ...
    closePrompt : function(caller,outside) {						// CLOSE PROMPT WHEN ERROR CORRECTED
      ...
      // ** Added by Scalpweb :
      $(caller).parent().removeClass('error');
      // **
    ...
    submitValidation : function(caller) {					// FORM SUBMIT VALIDATION LOOPING INLINE VALIDATION
      $.validationEngine.settings.preValidate($(caller));
      var stopForm = false;
      $.validationEngine.ajaxValid = true;
      ...
      if(stopForm || !$.validationEngine.ajaxValid){		// GET IF THERE IS AN ERROR OR NOT FROM THIS VALIDATION FUNCTIONS
        $.validationEngine.settings.failure && $.validationEngine.settings.failure($(caller));
